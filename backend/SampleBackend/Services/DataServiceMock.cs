...
{
    public class DataServiceMock : IArticlesDataService, ICommentsDataService
    {
        private static readonly List<Article> ArticlesSet = InitArticles().ToList();
        private static readonly List<Comment> CommentsSet = InitComments().ToList();

        public Article[] GetArticles()
        {
            return ArticlesSet.ToArray();
        }

        public Article GetArticle(string articleId)
        {
            return ArticlesSet.FirstOrDefault(x => x.Id == articleId);
        }

        public void ChangeRating(string articleId, bool isIncrease)
        {
            var article = GetArticle(articleId);
            var increment = isIncrease ? +1 : -1;
            if (article != null)
            {
                article.Rating = article.Rating + increment;
            }
        }

        public Comment[] GetComments(string articleId)
        {
            var result = new Comment[0];
            var commentsIds = GetArticle(articleId)?.CommentsIds;
            if (commentsIds != null && commentsIds.Any())
            {
                result = CommentsSet.Where(x => commentsIds.Contains(x.Id)).ToArray();
            }

            return result;
        }

        public string AddComment(string articleId, Comment comment)
        {
            comment.Id = Guid.NewGuid().ToString("N");
            CommentsSet.Add(comment);
            var article = GetArticle(articleId);
            var newId = new[] {comment.Id};
            if (article.CommentsIds == null || !article.CommentsIds.Any())
            {
                article.CommentsIds = newId;
            }
            else
            {
                var commentsIds = article.CommentsIds.Concat(newId).ToArray();
                article.CommentsIds = commentsIds;
            }

            return comment.Id;
        }

        private static IEnumerable<Comment> InitComments()
        {
    ...
    },
    {
        ...
        ""text"": ""Nisi sit nisi cillum dolor fugiat sint do nostrud ex cillum cupidatat. Culpa do duis non et excepteur labore dolor culpa qui tempor veniam. Ex labore deserunt qui sit aute ad incididunt do cupidatat eiusmod reprehenderit ad. Qui laborum qui voluptate velit et consectetur ipsum enim dolore minim. Est sint velit tempor reprehenderit. Qui consectetur ad minim consequat.""
            ...
            return JsonConvert.DeserializeObject<Comment[]>(comments);
        }

        private static IEnumerable<Article> InitArticles()
        {
            const string articles = @"[
    {
        ...
        ""commentsIds"": [""qwerqwer"", ""lkjhsdlfkg"", ""zxcvzcxvzxcv"", ""fghjfghjfghj"", ""ertyoertywte""],
        ""rating"": 32
    },
    {
        ...
        ""commentsIds"": [""qwerqwertyy"", ""sdfgsdfghu7u7urtv""],
        ""rating"": -30
    },
    {
        ...
        ""commentsIds"": [""kjfviudfv089w74"", ""r23uyrghasdfb7""],
        ""rating"": 26
    },
    {
        ...
        ""commentsIds"": [""rflsbvyeuwr334rg5"", ""234faasffasgfq3r3eda""],
        ""rating"": 15
    },
    {
        ...
        ""commentsIds"": [""23rfasdfasdvg"", ""fqasfd4r35tga""],
        ""rating"": 5
    },
    {
        ...
        ""commentsIds"": [""23qwfasdf3"", ""dadsafw4f45w"", ""234erasfdastw4a""],
        ""rating"": -5
    },
    {
        ...
        ""rating"": 0
            ...
            return JsonConvert.DeserializeObject<Article[]>(articles);
        }
    }
}